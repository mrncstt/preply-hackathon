...
export function InterviewModal({ onInterviewEnd }: InterviewModalProps) {
  ...
  // Auto-end after agent has sent 7+ final messages (wrap-up detected)
  useEffect(() => {
    if (isEnding) return;
    const agentMessages = messages.filter((m) => m.role === "agent" && m.final);
    if (agentMessages.length >= 7) {
      const lastAgent = agentMessages[agentMessages.length - 1]?.text?.toLowerCase() || "";
      const wrapUpPhrases = ["good picture", "got a good picture", "great path", "personalized plan", "build your", "shape a path", "enough info", "let me build"];
      const isWrapUp = wrapUpPhrases.some((phrase) => lastAgent.includes(phrase));
      if (isWrapUp || agentMessages.length >= 9) {
        // Wait 2s for the user to hear the wrap-up, then auto-end
    ...
    <div className="fixed inset-0 z-50 flex flex-col animate-fade-in" style={{ background: 'var(--background)' }}>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="hidden lg:block">
          <ThymiaSidebar isActive={isConnected} data={thymiaData} />
        </div>
      </div>
