...
export async function POST(req: NextRequest) {
  ...
  const llmApiKey = process.env.VOICE_LLM_API_KEY;
  const llmModel = process.env.VOICE_LLM_MODEL;
  const llmUrl = process.env.VOICE_LLM_URL;
  const llmStyle = process.env.VOICE_LLM_STYLE || "openai";
  const llmVendor = process.env.VOICE_LLM_VENDOR || "";
  const ttsVendor = process.env.VOICE_TTS_VENDOR;
  const ttsKey = process.env.VOICE_TTS_KEY;
  const ttsVoiceId = process.env.VOICE_TTS_VOICE_ID;
  const thymiaApiKey = process.env.VOICE_THYMIA_API_KEY || "";

  const missing = [];
  if (!appId) missing.push("VOICE_APP_ID");
  if (!appCertificate) missing.push("VOICE_APP_CERTIFICATE");
  if (!llmApiKey) missing.push("VOICE_LLM_API_KEY");
  if (!llmUrl) missing.push("VOICE_LLM_URL");
  if (!ttsVendor) missing.push("VOICE_TTS_VENDOR");
  if (!ttsKey) missing.push("VOICE_TTS_KEY");
  if (!ttsVoiceId) missing.push("VOICE_TTS_VOICE_ID");
  ...
  // Build LLM params — for custom vendor, include extra fields for audio subscriber + Thymia
  const llmParams: Record<string, any> = { model: llmModel };
  if (llmVendor === "custom") {
    const subscriberToken = await buildToken(channel, "5000", appId!, appCertificate!);
    const llmRtmUid = `5001-${channel}`;
    const rtmToken = await buildToken(channel, "5001", appId!, appCertificate!, llmRtmUid);
    llmParams.channel = channel;
    llmParams.app_id = appId;
    llmParams.user_uid = USER_UID;
    llmParams.agent_uid = AGENT_UID;
    llmParams.subscriber_token = subscriberToken;
    llmParams.rtm_token = rtmToken;
    llmParams.rtm_uid = llmRtmUid;
    if (thymiaApiKey) {
      llmParams.thymia_api_key = thymiaApiKey;
    }
  }
  ...
  const agentId = agoraData?.agent_id || agoraData?.id || null;

  // Register agent with custom LLM server (non-blocking) so it can start
  // audio subscriber + Thymia immediately
  if (llmVendor === "custom" && llmUrl) {
    const llmBase = llmUrl!.replace(/\/chat\/completions\/?$/, "");
    const registerUrl = `${llmBase}/register-agent`;
    fetch(registerUrl, {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        prompt,
        user_uid: llmParams.user_uid,
        subscriber_token: llmParams.subscriber_token,
        rtm_token: llmParams.rtm_token,
        rtm_uid: llmParams.rtm_uid,
        thymia_api_key: llmParams.thymia_api_key || null,
      }),
    }).catch((e) => console.error("[RegisterAgent] Failed:", e));
  }
