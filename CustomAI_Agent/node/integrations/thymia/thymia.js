...
// ─── WAX integration for biomarker storage ───
const WAX_HOST = process.env.WAX_HOST || '127.0.0.1';
const WAX_PORT = parseInt(process.env.WAX_PORT || '8080', 10);

function waxFactAdd(entity, attribute, value) {
  return new Promise((resolve, reject) => {
    const payload = JSON.stringify({
      jsonrpc: '2.0', id: Date.now(),
      method: 'fact.add',
      params: { entity, attribute, value, metadata: { source: 'thymia_biomarker' } }
    });
    const req = http.request({
      hostname: WAX_HOST, port: WAX_PORT, path: '/', method: 'POST',
...
function saveBiomarkersToWax(appId, channel, metrics) {
  const entity = `user:${appId}`;
  const bio = metrics.biomarkers || {};
  const parts = [];
  for (const [k, v] of Object.entries(bio)) {
    if (v === null || v === undefined || typeof v !== 'number' || Math.abs(v) < 0.001) continue;
  ...
  const summary = `Session ${new Date().toISOString().slice(0, 16)}: ${parts.join(', ')}`;
  waxFactAdd(entity, 'biomarker_snapshot', summary).then(() => {
    logger.info(`[WAX] Saved biomarker snapshot for ${entity}`);
  }).catch(e => {
    logger.debug(`[WAX] Failed to save biomarker: ${e.message}`);
  });

  // Save safety alerts as separate facts
  const safety = metrics.safety || {};
  if (safety.alert && safety.alert !== 'none') {
    const alertValue = `${safety.alert}: ${safety.rationale || 'no details'}`;
    waxFactAdd(entity, 'biomarker_alert', alertValue).catch(() => {});
  }
}
  ...
  const client = new ThymiaClient({
    ...
    onPolicyResult: (result) => {
      ...
      if (metrics && hasNonNullScores(metrics.biomarkers)) {
        ...
        // Save to WAX for long-term memory
        saveBiomarkersToWax(appId, channel, metrics);
      } else {
...
module.exports = {
  ...
  onRequest(ctx) {
    ...
    // Check for browser-side Thymia biomarkers in messages (sent via RTM → forwarded by engine)
    for (const msg of (messages || [])) {
      if (msg.role === 'user' && typeof msg.content === 'string') {
        try {
          const parsed = JSON.parse(msg.content);
          if (parsed.object === 'thymia.biomarkers.client') {
            const st = getOrCreateState(appId, channel);
            st.clientBiomarkers = {
              biomarkers: parsed.biomarkers || {},
              alert: parsed.alert || 'none',
              rationale: parsed.rationale || '',
              timestamp: parsed.timestamp,
            };
            logger.info(`[ClientBiomarkers] Received from browser for ${key}: ${Object.keys(parsed.biomarkers || {}).length} scores`);
            // Save to WAX
            saveBiomarkersToWax(appId, channel, { biomarkers: parsed.biomarkers || {}, safety: { alert: parsed.alert } });
          }
        } catch (e) {
          // Not JSON, ignore
        }
  ...
  /**
   * Inject biomarkers from browser-side Thymia into the LLM system prompt.
   * Called on each /chat/completions request.
   */
  getSystemInjection(appId, channel) {
    const key = getKey(appId, channel);
    const state = channelState.get(key);
    if (!state || !state.clientBiomarkers) return null;

    const bio = state.clientBiomarkers;
    const parts = [];
    for (const [k, v] of Object.entries(bio.biomarkers || {})) {
      if (v === null || v === undefined || typeof v !== 'number' || Math.abs(v) < 0.001) continue;
      const pct = Math.round(v * 100);
      const label = k.replace(/_/g, ' ');
      parts.push(`${label}: ${pct}%`);
    ...
    let injection = `[Voice Biomarker Update from Thymia] Live voice analysis scores:\n${parts.join(', ')}\n`;
    if (bio.alert && bio.alert !== 'none' && bio.alert !== 'passthrough') {
      injection += `Alert: ${bio.alert}\n`;
    }
    if (bio.rationale) {
      injection += `Analysis: ${bio.rationale}\n`;
    }
    injection += `Use these insights naturally. Don't list numbers - describe what the data suggests about the user's emotional state.`;

    return { role: 'system', content: injection };
  },
