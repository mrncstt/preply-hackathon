...
const WAX_ENABLED = process.env.WAX_ENABLED === 'true';
if (WAX_ENABLED) {
  const waxModule = require('./integrations/wax/wax');
  waxModule.init(audioSubscriber, { rtmClient: () => rtmClient });
  modules.push(waxModule);
}
 ...
 */
async function executeTools(toolCalls, appId, userId, channel) {
  const results = [];
  for (const tc of toolCalls) {
    ...
    try {
      let result = fn(appId, userId, channel, args);
      // Await if handler is async
      if (result && typeof result.then === 'function') {
        result = await result;
      }
      // OpenAI requires content to be a string
      const content = typeof result === 'string' ? result : JSON.stringify(result);
      results.push({ role: 'tool', tool_call_id: tcId, name, content });
    } catch (e) {
  ...
  try {
    ...
    for (const mod of modules) {
      if (mod.getSystemInjection) {
        const injection = await mod.getSystemInjection(appId, channel);
